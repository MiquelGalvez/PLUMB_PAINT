...
public class RedGbotMoves : MonoBehaviour
{
    public float speed;
    ...
    private void Update()
    {
        ...
        if (esDerecha)
        {
            ...
            if (transform.eulerAngles.y != 180) // Si no está mirando a la derecha
            {
                transform.rotation = Quaternion.Euler(0, 180, 0); // Rotar hacia la derecha
            }
        }
        else
        {
            transform.position += Vector3.left * speed * Time.deltaTime;
            if (transform.eulerAngles.y != 0) // Si no está mirando a la izquierda
            {
                transform.rotation = Quaternion.Euler(0, 0, 0); // Rotar hacia la izquierda
            }
        }
